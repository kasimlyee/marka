...
class MarkaApplication {
  ...
  setupIpcHandlers() {
    ...
    ipcMain.handle("reports-generate-bulk", async (event, reportCriteria) => {
      try {
        const results = await this.reportGenerator.generateBulkReports(
          reportCriteria
        );
  ...
  async handleExportStudents(format) {
    try {
      ...
      if (!result.canceled) {
        const exportPath = await this.fileManager.exportStudents(
          result.filePath,
          format
        );
