  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const gradingSchemas = await window.electronAPI.settings.get('gradingSchemas') || {};
        setGradingSchemas(gradingSchemas);
        setActiveSchema(await window.electronAPI.settings.get('activeSchema') || 'PLE');
...
console.log("window.electronAPI:", window.electronAPI);
  ...
  const handleRestoreDatabase = async () => {
    try {
      ...
      if (!result.canceled && result.filePaths.length > 0) {
        const confirmed = window.confirm('Are you sure you want to restore from backup? This will replace all current data.');
        if (confirmed) {
          const restoreResult = await window.electronAPI.backup.restore(result.filePaths[0]);
          if (restoreResult.success) {
            showNotification('Database restored successfully. Application will restart.', 'success');
  ...
  const saveStudent = async (studentData) => {
    try {
      let result;
      if (studentData.id) {
        result = await window.electronAPI.students.update(studentData.id, studentData);
      } else {
      ...
      if (result) {
        ...
        showNotification(`Student ${studentData.id ? 'updated' : 'added'} successfully`, 'success');
        return true;
      }
    } catch (error) {
      showNotification(`Failed to ${studentData.id ? 'update' : 'add'} student`, 'error');
  ...
  const saveSubject = async (subjectData) => {
    try {
      let result;
      if (subjectData.id) {
        result = await window.electronAPI.subjects.update(subjectData.id, subjectData);
      } else {
      ...
      if (result) {
        ...
        showNotification(`Subject ${subjectData.id ? 'updated' : 'added'} successfully`, 'success');
        return true;
      }
    } catch (error) {
      showNotification(`Failed to ${subjectData.id ? 'update' : 'add'} subject`, 'error');
  ...
  // Components
  const Sidebar = () => (
    <div className="w-64 bg-white shadow-lg h-full flex flex-col" style={{ borderRight: `2px solid ${theme.secondary}` }}>
      <div className="p-6 border-b" style={{ borderColor: theme.secondary }}>
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: theme.secondary }}>
            <span className="text-white font-bold text-xl">M</span>
          </div>
          <div>
            <h1 className="text-xl font-bold" style={{ color: theme.primary }}>Marka</h1>
      ...
      <nav className="flex-1 p-4">
        <div className="space-y-2">
          {[
            ...
            { id: 'analytics', icon: TrendingUp, label: 'Analytics', locked: licenseInfo.type === 'Standard' },
            { id: 'users', icon: UserPlus, label: 'User Management', locked: licenseInfo.type !== 'Enterprise' },
            ...
            <button
              key={item.id}
              onClick={() => item.locked ? 
                showNotification(`Feature requires ${item.id === 'analytics' ? 'Pro' : 'Enterprise'} license`, 'warning') : 
              ...
              style={{ 
                backgroundColor: currentView === item.id ? theme.secondary : 'transparent'
              }}
            >
              <div className="flex items-center space-x-3">
      ...
      <div className="p-4 border-t">
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <p className="text-xs text-gray-600">Expires: {licenseInfo.expiryDate || 'N/A'}</p>
          <div className="flex items-center space-x-1 mt-1">
            <div className={`w-2 h-2 rounded-full ${licenseInfo.isActivated ? 'bg-green-500' : 'bg-red-500'}`}></div>
  ...
  const Dashboard = () => {
    ...
    const averagePerformance = totalStudents > 0 ? Math.round(
    ...
    const highPerformers = students.filter(s => {
      const avg = Object.values(s.subjects).reduce((sum, scores) => {
        const score = typeof scores === 'object' ? (scores.term1 + scores.term2 + scores.term3) / 3 : scores;
        return sum + score;
    ...
    return (
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Comprehensive school management dashboard</p>
        </div>

        {isLoading ? (
          <div className="flex justify-center items-center h-64">
            <Loader2 className="w-8 h-8 animate-spin" style={{ color: theme.secondary }} />
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {[
                { label: 'Total Students', value: totalStudents, icon: Users, color: theme.secondary, trend: '+12%' },
                { label: 'Active Classes', value: getClassList().length - 1, icon: GraduationCap, color: theme.accent, trend: '+2%' },
                { label: 'Average Performance', value: `${averagePerformance}%`, icon: TrendingUp, color: theme.primary, trend: '+5%' },
                { label: 'High Performers', value: highPerformers, icon: Award, color: theme.success, trend: '+18%' }
              ].map((stat, index) => (
                <div key={index} className="bg-white rounded-lg shadow-sm p-6 border-l-4" style={{ borderColor: stat.color }}>
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-xs px-2 py-1 rounded" style={{ backgroundColor: `${stat.color}20`, color: stat.color }}>
                      {stat.trend}
                    </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Recent Activity</h3>
                  ...
                  {[
                    ...
                    <div key={index} className="flex items-center space-x-4 p-3 rounded-lg bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        {activity.student && <p className="text-sm text-gray-600">{activity.student}</p>}
                      </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Quick Actions</h3>
                <div className="space-y-3">
                  {[
                    { label: 'Add Student', icon: UserPlus, action: () => openModal('addStudent') },
                    { label: 'Generate Reports', icon: FileText, action: () => setCurrentView('reports') },
                    { label: 'Export Data', icon: Download, action: exportStudentData },
                    { label: 'System Backup', icon: Database, action: handleBackupDatabase }
                  ].map((action, index) => (
                    <button
                      key={index}
                    ...
                    >
                      <action.icon className="w-5 h-5" style={{ color: theme.secondary }} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Class Performance Overview</h3>
                <div className="space-y-3">
                  {getClassList().filter(c => c !== 'All').map(className => {
                    ...
                    return (
                      <div key={className} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                        <div>
                          ...
                          <span className="text-sm text-gray-600 ml-2">({classStudents.length} students)</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <div className="w-20 bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              style={{ 
                                width: `${avgScore}%`, 
                                backgroundColor: avgScore >= 85 ? theme.success : avgScore >= 70 ? theme.accent : theme.error 
                              }}
                            ></div>
                          </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>System Status</h3>
                <div className="space-y-4">
                  {[
                    { label: 'Database Connection', status: 'Connected', icon: Database, color: theme.success },
                    { label: 'License Status', status: licenseInfo.isActivated ? 'Active' : 'Inactive', icon: Shield, color: licenseInfo.isActivated ? theme.success : theme.error },
                    { label: 'Cloud Sync', status: licenseInfo.type === 'Enterprise' ? 'Enabled' : 'Disabled', icon: Cloud, color: licenseInfo.type === 'Enterprise' ? theme.success : theme.warning },
                    { label: 'Local Storage', status: 'Healthy', icon: HardDrive, color: theme.success },
                    { label: 'Last Backup', status: '2 days ago', icon: RefreshCw, color: theme.accent }
                  ].map((item, index) => (
  ...
  const StudentsView = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold" style={{ color: theme.primary }}>Students Management</h2>
            ...
            <div className="flex space-x-4">
              ...
              >
                {getClassList().map(className => (
                  <option key={className} value={className}>{className === 'All' ? 'All Classes' : `Class ${className}`}</option>
                ))}
              </select>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredStudents.length > 0 ? (
                  filteredStudents.map(student => {
                    const grade = calculateGrade(student.subjects, student.class.startsWith('P') ? 'PLE' : 'UCE');
                    return (
                            ...
                            <div className="w-16 bg-gray-200 rounded-full h-2">
                              <div 
                                ...
                                style={{ 
                                  ...
                                  backgroundColor: student.attendance >= 90 ? theme.success : student.attendance >= 75 ? theme.accent : theme.error 
                                }}
                              ></div>
                            </div>
  ...
  const SubjectsView = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold" style={{ color: theme.primary }}>Subjects Management</h2>
        <button
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {getClassList().filter(c => c !== 'All').map(className => (
            <div key={className} className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Class {className} Subjects</h3>
              <div className="space-y-3">
                {subjects.filter(subject => subject.class === className).map(subject => (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {subject.isCore ? 'Core Subject' : 'Elective'} • Weight: {subject.weight}
                        </p>
                      </div>
                    </div>
                    <div className="flex space-x-2">
  ...
  const ReportsView = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold" style={{ color: theme.primary }}>Report Generation</h2>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Bulk Report Generation</h3>
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Report Term</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent">
              ...
              <button 
                onClick={async () => {
                  try {
                    const result = await window.electronAPI.reports.generateBulk({
                      class: 'All',
                      term: 'term3'
                    });
                    
                    if (result.success) {
                      showNotification('Bulk reports generated successfully', 'success');
                    } else {
                      showNotification(`Bulk report generation failed: ${result.error}`, 'error');
                    }
                  } catch (error) {
                    showNotification('Failed to generate bulk reports', 'error');
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Individual Reports</h3>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Report Templates</h3>
              <div className="space-y-3">
                {[
                  { 
                  ...
                  { 
                    ...
                    description: 'Standard Uganda Certificate of Education format',
                    preview: true
                  },
                  { 
                  ...
                  <div key={index} className={`p-4 border rounded-lg ${template.locked ? 'bg-gray-50' : 'hover:shadow-md cursor-pointer'}`}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      {template.locked && (
                        <span className="text-xs px-2 py-1 bg-gray-200 text-gray-600 rounded">
                          {template.name.includes('Custom') ? 'Pro' : 'Enterprise'}
                        </span>
                      )}
                    </div>
                    {template.preview && (
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Report Statistics</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={index} className="flex justify-between items-center">
                    ...
                    <span className="font-semibold" style={{ color: theme.primary }}>{stat.value}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  const AnalyticsView = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold" style={{ color: theme.primary }}>Performance Analytics</h2>
        <div className="flex space-x-3">
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
            {[
              { title: 'Class Performance Trends', chart: 'line', color: theme.secondary },
              { title: 'Subject Distribution', chart: 'pie', color: theme.accent },
              { title: 'Grade Distribution', chart: 'bar', color: theme.primary }
            ].map((chart, index) => (
              <div key={index} className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>{chart.title}</h3>
                <div className="h-48 flex items-center justify-center border-2 border-dashed border-gray-200 rounded">
                  <div className="text-center">
                    <BarChart3 className="w-12 h-12 mx-auto mb-2 text-gray-400" />
                    <p className="text-sm text-gray-500">Interactive {chart.chart} chart would appear here</p>
                    <p className="text-xs text-gray-400 mt-1">Data visualization with Chart.js/D3</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Top Performers</h3>
              <div className="space-y-3">
                {students
                  .sort((a, b) => {
                    const avgA = Object.values(a.subjects).reduce((sum, scores) => {
                      const score = typeof scores === 'object' ? (scores.term1 + scores.term2 + scores.term3) / 3 : scores;
                      return sum + score;
                    }, 0) / Object.keys(a.subjects).length;
                    const avgB = Object.values(b.subjects).reduce((sum, scores) => {
                      const score = typeof scores === 'object' ? (scores.term1 + scores.term2 + scores.term3) / 3 : scores;
                      return sum + score;
                  ...
                  .slice(0, 5)
                  .map((student, index) => {
                    const avg = Math.round(Object.values(student.subjects).reduce((sum, scores) => {
                      const score = typeof scores === 'object' ? (scores.term1 + scores.term2 + scores.term3) / 3 : scores;
                      return sum + score;
                      ...
                      <div key={student.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                        <div className="flex items-center space-x-3">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold"
                              style={{ backgroundColor: index === 0 ? '#FFD700' : index === 1 ? '#C0C0C0' : index === 2 ? '#CD7F32' : theme.secondary }}>
                            {index + 1}
                          </div>
                          <div>
                            <p className="font-medium">{student.name}</p>
                        ...
                        <div className="text-right">
                          <p className="font-semibold" style={{ color: theme.primary }}>{avg}%</p>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Subject Performance Analysis</h3>
              <div className="space-y-4">
                {['English', 'Mathematics', 'Science', 'Social Studies'].map(subject => {
                  const subjectScores = students
                    ...
                    .map(s => {
                      ...
                      return typeof scores === 'object' ? (scores.term1 + scores.term2 + scores.term3) / 3 : scores;
                    });
                  const avg = subjectScores.length > 0 ? Math.round(subjectScores.reduce((a, b) => a + b, 0) / subjectScores.length) : 0;
                  
                  return (
                    <div key={subject}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-medium">{subject}</span>
                        <span className="text-sm" style={{ color: theme.primary }}>{avg}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${avg}%`, 
                            backgroundColor: avg >= 85 ? theme.success : avg >= 70 ? theme.accent : theme.error 
                          }}
                        ></div>
                      </div>
                    </div>
  ...
  const UsersView = () => (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold" style={{ color: theme.primary }}>User Management</h2>
        <button
  ...
  const SettingsView = () => (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6" style={{ color: theme.primary }}>Settings</h2>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>School Information</h3>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setSchoolData({...schoolData, founded: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent"
                    />
                  </div>
                </div>
                ...
                <button
                  onClick={async () => {
                    const success = await saveSettings('schoolData', schoolData);
                    if (success) {
                      const updatedSettings = await window.electronAPI.settings.getAll();
                      setSettings(updatedSettings);
                    }
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Grading System Configuration</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Active Grading Schema</label>
                  <select 
                    value={activeSchema}
                    onChange={(e) => {
                  ...
                  >
                    {Object.keys(gradingSchemas).map(schema => (
                      <option key={schema} value={schema}>{gradingSchemas[schema].name}</option>
                    ))}
                  </select>
                </div>
                <div className="p-4 border border-dashed border-gray-300 rounded-lg text-center">
                  <Upload className="w-8 h-8 mx-auto mb-2 text-gray-400" />
                  <p className="text-sm text-gray-600">Upload Custom Grading Schema (JSON)</p>
                  <p className="text-xs text-gray-500 mt-1">
                    {licenseInfo.type === 'Standard' ? 'Pro and Enterprise tiers only' : 'Click to upload or drag and drop'}
                  </p>
                  {licenseInfo.type !== 'Standard' && (
                    <button 
                      onClick={() => showNotification('JSON schema upload functionality would be implemented here', 'info')}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>System Preferences</h3>
              <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Data Management</h3>
              <div className="space-y-3">
                <button
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Security Settings</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
                  ...
                  <span className="text-sm px-2 py-1 bg-orange-100 text-orange-800 rounded">
                    {licenseInfo.type === 'Enterprise' ? 'Available' : 'Enterprise Only'}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded">
                  <div>
  ...
  const LicenseView = () => (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6" style={{ color: theme.primary }}>License Management</h2>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold" style={{ color: theme.primary }}>Current License</h3>
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                  licenseInfo.isActivated ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
                  {licenseInfo.isActivated ? 'Active' : 'Inactive'}
                </span>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-lg" style={{ color: theme.primary }}>{licenseInfo.type}</p>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <code className="px-2 py-1 bg-gray-100 rounded text-sm font-mono">{licenseInfo.machineId}</code>
                    <button 
                      onClick={() => {
                        navigator.clipboard.writeText(licenseInfo.machineId);
                        showNotification('Machine ID copied to clipboard', 'success');
                      }}
                ...
                <div className="pt-4 border-t">
                  <h4 className="font-semibold mb-3" style={{ color: theme.primary }}>Current Plan Features</h4>
                  <div className="space-y-2">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>License Actions</h3>
              <div className="space-y-3">
                <button
                  onClick={async () => {
                    try {
                      ...
                      if (result) {
                        showNotification('License validation successful', 'success');
                        const updatedLicense = await window.electronAPI.license.getInfo();
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Available Plans</h3>
              <div className="space-y-4">
                {[
                  {
                    name: 'Standard',
                  ...
                  <div key={index} className={`border-2 rounded-lg p-4 ${
                    plan.name === licenseInfo.type ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-lg">{plan.name}</h4>
                      <span className="text-sm font-medium" style={{ color: theme.secondary }}>{plan.price}</span>
                    </div>
                    <div className="space-y-1 mb-3">
                      {plan.features.slice(0, 3).map((feature, index) => (
                        <div key={index} className="flex items-center space-x-2">
                          <CheckCircle className="w-3 h-3" style={{ color: theme.success }} />
                    ...
                    {plan.name !== licenseInfo.type && (
                      <button
                        onClick={async () => {
                          try {
                            const result = await window.electronAPI.license.activate(plan.name);
                            if (result) {
                              showNotification(`Upgraded to ${plan.name} license`, 'success');
                              const updatedLicense = await window.electronAPI.license.getInfo();
                              setLicenseInfo(updatedLicense);
                            } else {
                              showNotification(`Failed to upgrade to ${plan.name}`, 'error');
                            }
                          } catch (error) {
                      ...
                      >
                        {plan.name === 'Standard' ? 'Downgrade' : 'Upgrade'} to {plan.name}
                      </button>
                    )}
                    {plan.name === licenseInfo.type && (
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold mb-4" style={{ color: theme.primary }}>Usage Statistics</h3>
              <div className="space-y-4">
                {[
                  { 
                    label: 'Students', 
                  ...
                  <div key={index}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="h-2 rounded-full transition-all duration-300" 
                        style={{ 
                          width: usage.unlimited ? '20%' : `${Math.min((usage.current / usage.limit) * 100, 100)}%`,
                          backgroundColor: usage.unlimited ? theme.success : 
                            (usage.current / usage.limit) < 0.8 ? theme.success : 
                            (usage.current / usage.limit) < 0.95 ? theme.warning : theme.error
                        }}
                      ></div>
                    </div>
                  </div>
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          />
          <label htmlFor="isCore" className="text-sm font-medium text-gray-700">Core Subject</label>
        </div>
        <div className="flex space-x-3 pt-4">
          <button
            type="button"
  ...
  const EditSubjectModal = () => {
    const [formData, setFormData] = useState(selectedSubject || {
      name: '',
      class: 'P7',
      isCore: true,
      weight: 1.0
  ...
  const ImportStudentsModal = () => {
    ...
    const handleImport = async () => {
      ...
      try {
        ...
        if (result.success) {
          showNotification(`${result.data.length} students imported successfully`, 'success');
          const updatedStudents = await window.electronAPI.students.getAll();
          setStudents(updatedStudents);
          closeModal();
        } else {
    ...
    return (
      <div className="space-y-4">
        <div className="p-4 border border-dashed border-gray-300 rounded-lg text-center">
          <Upload className="w-8 h-8 mx-auto mb-2 text-gray-400" />
          <p className="text-sm text-gray-600">Upload CSV or Excel file with student data</p>
          <input
            type="file"
            accept=".csv,.xlsx,.xls"
  ...
  return (
    <div className="flex h-screen" style={{ backgroundColor: theme.background }}>
        ...
        <header className="bg-white shadow-sm border-b px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-lg font-semibold" style={{ color: theme.primary }}>
                  {currentView.charAt(0).toUpperCase() + currentView.slice(1)}
                </h1>
                <p className="text-sm text-gray-600">
                  {currentView === 'dashboard' && 'Overview of your school management system'}
                  {currentView === 'students' && `${filteredStudents.length} students found`}
                  {currentView === 'subjects' && `${subjects.length} subjects configured`}
                  {currentView === 'reports' && 'Generate and manage report cards'}
                  ...
                  {currentView === 'license' && 'License information and management'}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-right">
                <p className="text-sm font-medium" style={{ color: theme.primary }}>{schoolData.name || 'Marka'}</p>
                <p className="text-xs text-gray-600">{licenseInfo.type || 'Loading'} License • {students.length} Students</p>
              </div>
      ...
      {/* Modals */}
      <Modal isOpen={showModal && modalType === 'addStudent'} onClose={closeModal} title="Add New Student">
        <AddStudentModal />
      </Modal>

      <Modal isOpen={showModal && modalType === 'editStudent'} onClose={closeModal} title="Edit Student">
        <EditStudentModal />
      </Modal>

      <Modal isOpen={showModal && modalType === 'addSubject'} onClose={closeModal} title="Add New Subject">
        <AddSubjectModal />
      </Modal>

      <Modal isOpen={showModal && modalType === 'editSubject'} onClose={closeModal} title="Edit Subject">
        <EditSubjectModal />
      </Modal>

      <Modal isOpen={showModal && modalType === 'importStudents'} onClose={closeModal} title="Import Students">
        <ImportStudentsModal />
      </Modal>
      ...
      {notification && (
        <div className="fixed top-4 right-4 z-50">
          <div className={`flex items-center space-x-2 px-4 py-3 rounded-lg shadow-lg text-white max-w-sm ${
